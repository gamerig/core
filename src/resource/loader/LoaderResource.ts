 ...
 */
function setExtMap(map: Record<string, any>, extname: string, val: number) {
...
class LoaderResource {
  ...
  private _xhrOnLoad(): void {
    ...
    if (
      status === STATUS_NONE &&
      (text.length > 0 || xhr.responseType === LoaderResource.XHR_RESPONSE_TYPE.BUFFER)
    ) {
      status = STATUS_OK;
    }
...
namespace LoaderResource {
  ...
  export const _xhrTypeMap: Record<string, XHR_RESPONSE_TYPE> = {
    // xml
